...
from satori.error import ClientError, ServerError
from satori.lib.utils import get_timestamp
from satori.lib.utils import cleanNoneValue
from satori.lib.utils import encoded_string,decoded_string
from satori.lib.utils import check_required_parameter
from satori.lib.authentication import hmac_hashing, rsa_signature, ed25519_signature
...
class API(object):
    ...
    Keyword Args:
        base_url (str, optional): the API base url, useful to switch to testnet, etc. By default it's https://api.satori.com
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "satori-connector-python/" + __version__,
                "APIKEY": api_key,
            }
        )
    ...
    def sign_request(self, http_method, url_path, payload=None):
        ...
        sign_string = decoded_string(query_string)
        self.session.headers.update({
            "signature": self._get_sign(sign_string),
